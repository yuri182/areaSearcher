...
function sortColor(figure){
    ...
    if(figure.slice(0,1) == "A"){
        ...
        //これもし数字がない場合どうなります?
        //1文字目(英文字1文字)しかない場合でも正常に動きます?この場合、1文字しかないものをslice(0,1)しても1文字目が取れるだけなので、変わらないです（0文字だと死ぬ）なるほどそっか。（いまさら2階以上がないエリアの存在に気づく始末）あ、数字は階数です。なるほど
...
window.addEventListener('DOMContentLoaded', function(){ // これでこの中の処理がHTMLの要素が全部読み込まれてから動くことになりますok
    ...
    document.getElementById('box').addEventListener('box', inputFilter); //
